...
use AppBundle\Form\Type\TrainingsstageTrainerType;
use Symfony\Component\HttpFoundation\File\UploadedFile;
use AppBundle\Entity\TrainingsstageTurnster;
use AppBundle\Form\Type\TrainingsstageTurnsterType;
...
class GetContentController extends BaseController
{
	...
	/**
     * @Route("/trainingsstage/", name="trainingsstage")
     * @Method({"GET", "POST"})
     */
    public function trainingsstage(Request $request)
    {
        ...
        if ($request->query->get('as') === 'turnster') {
            ...
            $form                   = $this->createForm(new TrainingsstageTurnsterType, $trainingsstageTurnster);
        ...
        if ($request->query->get('as') === 'trainer') {
            ...
            $form                  = $this->createForm(new TrainingsstageTrainerType(), $trainingsstageTrainer);
            $form->handleRequest($request);
            if ($form->isSubmitted() && $form->isValid()) {
                /** @var UploadedFile $file */
    ...
    /**
     * @Route("/trainingsstageSuccess/", name="trainingsstageSuccess")
     * @Method({"GET"})
     */
