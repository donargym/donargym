...
class SelectieController extends BaseController
{
                ...
                for ($i = 0; $i < count($functies); $i++) {
                    ...
                    for ($j = 0; $j < count($groepFuncties); $j++) {
                        if ($groepFuncties[$j]->getFunctie() == 'Turnster') {
                            ...
                            $persoonItems->functies[$i]->turnster[$j]->percentageVoortgang
                                                                                                = $turnster->getVoortgangTotaal(
                            );
                            $persoonItems->functies[$i]->turnster[$j]->updatedAt
                                                                                                = $turnster->getUpdatedCijfersAt(
                            );
                            $persoonItems->functies[$i]->turnster[$j]->percentageVoortgangKleur = $this->colorGenerator(
                                $persoonItems->functies[$i]->turnster[$j]->percentageVoortgang
                            );
                            if ($page == 'Index') {
                                $aantalAanwezig     = 0;
                            ...
                            $response                                                = $this->getDoelCijfersForTurnster(
                                $turnster->getId()
                            );
                            ...
                            if ($doelprijzen) {
                                ...
                                foreach ($doelIds as $doelId) {
                                    $check = false;
                                    if ($result = $this->getDoelVoorPrijzenTachtigProcent(
                                        $doelId,
                                        $turnster->getId()
                                    )) {
                                        $persoonItems->functies[$i]->turnster[$j]->doelenPrijzen[$doelCounter]
                                            = new \stdClass();
                                        $persoonItems->functies[$i]->turnster[$j]->doelenPrijzen[$doelCounter]->tachtigProcent
                                            = $result;
                                        $countDoelenPrijzen++;
                                        $countMedailles++;
                                        $check = true;
                                    }
                                    if ($result = $this->getDoelVoorPrijzenNegentigProcent(
                                        $doelId,
                                        $turnster->getId()
                                    )) {
                                        $persoonItems->functies[$i]->turnster[$j]->doelenPrijzen[$doelCounter]
                                            = new \stdClass();
                                        $persoonItems->functies[$i]->turnster[$j]->doelenPrijzen[$doelCounter]->negentigProcent
                                            = $result;
                                        $countDoelenPrijzen++;
                                        $countBekers++;
                                        $check = true;
    ...
    private function getDoelVoorPrijzenTachtigProcent($doelId, $turnsterId)
    {
        ...
        $query   = $em->createQuery(
            ...
            FROM App:SeizoensDoelen seizoensdoelen
     ...
     * @Route("/inloggen/selectie/showDoelPrijzen/{persoonId}/{groepId}/", name="showDoelPrijzen", methods={"GET"})
     */
    public
            ...
            foreach ($people as $person) {
                ...
                foreach ($functies as $functie) {
                    ...
                    if ($functie->getFunctie() == 'Turnster') {
                        ...
                        $turnsters[$number]->naam                         = $person->getVoornaam(
                            ) . ' ' . $person->getAchternaam();
    ...
    /**
     * @IsGranted("ROLE_TRAINER")
     * @Route("/inloggen/selectie/showDoelPrijzen/{persoonId}/{groepId}/updateDoelPrijs/{doelId}/{percentage}/", name="updateDoelPrijzen", methods={"GET"})
     */
    public
    ...
    /**
     * @IsGranted("ROLE_TRAINER")
     * @Route("/inloggen/selectie/showDoelPrijzen/{persoonId}/{groepId}/undoDoelPrijs/{doelId}/{percentage}/", name="undoDoelPrijzen", methods={"GET"})
     */
    public
    ...
    /**
     * @IsGranted("ROLE_TURNSTER")
     * @Route("/inloggen/selectie/{persoonId}/Doelen/{toestel}/", name="showPersoonDoelenPerToestel", methods={"GET"})
     */
    ...
    /**
     * @IsGranted("ROLE_TURNSTER")
     * @Route("/inloggen/selectie/{persoonId}/Doelen/{toestel}/showDoel/{doelId}/", name="showPersoonOneDoelPerToestel", methods={"GET"})
     */
        ...
        foreach ($percentages as $percentage) {
            ...
            if ($percentage == 80) {
                ...
                foreach ($seizoensdoelenObject as $seizoensdoelObject) {
                    ...
                    $doelenTachtigProcentGegeven[$seizoensdoelObject->getId()]['cijfer']
                                                                                           = $seizoensdoelObject->getCijfer(
                    );
                ...
                foreach ($seizoensdoelenObject as $seizoensdoelObject) {
                    ...
                    $doelenNegentigProcentGegeven[$seizoensdoelObject->getId()]['cijfer']
                                                                                            = $seizoensdoelObject->getCijfer(
                    );
                    $doelenNegentigProcentGegeven[$seizoensdoelObject->getId()]['id']
                                                                                            = $seizoensdoelObject->getId(
                    );
                }
            }
        }
        return array(
            $doelenTachtigProcent,
            $doelenNegentigProcent,
            $doelenIds,
            $aantalMedailles,
            $aantalBekers,
            $doelenTachtigProcentGegeven,
            $doelenNegentigProcentGegeven
        );
    }
    ...
    /**
     * @IsGranted("ROLE_ASSISTENT")
     * @Route("/inloggen/selectie/{persoonId}/viewTurnster/{turnsterId}/{groepId}/viewOneDoel/{doelId}/", name="viewSelectieTurnsterOneDoel", methods={"GET"})
     */
    public
    ...
    protected function getPercentages($doel, $cijfers, $turnsterId)
    {
        ...
        if (count($subdoelPercentages) > 0) {
            $doelPercentage = 0.6 * (array_sum($subdoelPercentages) / count(
                        ($subdoelPercentages)
                    )) + 0.4 * $hoofddoelPercentage;
        } else {
    ...
    /**
     * @IsGranted("ROLE_TRAINER")
     * @Route("/inloggen/selectie/{persoonId}/viewTurnster/{turnsterId}/{groepId}/cijferGeven/", name="SelectieTurnsterAddCijfer", methods={"GET", "POST"})
     */
    public
    ...
    /**
     * @IsGranted("ROLE_ASSISTENT")
     * @Route("/inloggen/selectie/{persoonId}/addDoelToTurnster/{groepId}/{turnsterId}/", name="addDoelToTurnster", methods={"GET", "POST"})
     */
    ...
    /**
     * @IsGranted("ROLE_ASSISTENT")
     * @Route("/inloggen/selectie/{persoonId}/addDoelToTurnster/{groepId}/{turnsterId}/{doelId}/", name="removeDoelFromTurnster", methods={"GET", "POST"})
     */
    ...
    /**
     * @IsGranted("ROLE_ASSISTENT")
     * @Route("/inloggen/selectie/{persoonId}/viewdoelen/{groepId}/", name="viewDoelen", methods={"GET"})
     */
    ...
    /**
     * @IsGranted("ROLE_TRAINER")
     * @Route("/inloggen/selectie/{persoonId}/adddoelen/{groepId}/", name="addDoelen", methods={"GET", "POST"})
     */
            ...
            if ($doelObject->getTrede()) {
                ...
                for ($trede; $trede > 0; $trede--) {
                    ...
                    for ($j = 0; $j < count($subdoelenArray); $j++) {
                        ...
                        $trededoelObject                                         = $query->setMaxResults(1)
                            ->getOneOrNullResult();
                        ...
                        if (count($subsubdoelenIds) > 0) {
                            ...
                            if (count($subsubdoeltrede) == 2) {
                                if (!($subsubdoeltrede[1] == ($trede - 1) && $subsubdoelObject->getToestel(
                                    ) == $hoofddoel->toestel
                                    && $subsubdoelObject->getNaam() == $hoofddoel->naam)) {
                                    $doelOpbouw->subdoelen[$trede]->trededoelen[$j]->subdoelId
                                        = $trededoelObject->getId();
                                    if (!in_array($trededoelObject->getId(), $subDoelIds)) {
                                        $subDoelIds[] = $trededoelObject->getId();
                                    }
                                }
    ...
    /**
     * @IsGranted("ROLE_ASSISTENT")
     * @Route("/inloggen/selectie/{persoonId}/viewonedoel/{groepId}/{doelId}/", name="viewOneDoel", methods={"GET"})
     */
